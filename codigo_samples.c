...
//#define SEED 316753
#define D  100 // acoplamiento
#define s2 10.0 //intensidad de ruído
#define tmax 2e7
#define test 8e5
#define samples 7 // numero de muestras independientes
...
int main(void){
  ...
  char nome[1000]; // de la misma manera que la linea anterior
  sprintf(nome,"dados_L%d_tmax%.1e_test%.1e_sigma%.3f.data",L,tmax,test,s2); //coloque para definir los valores en el aquivo
  FILE *f;
  f = fopen(nome,"w");
...
// Calculo de las medias finales y barras de error  
  ...
  fprintf(f,"#valor do devio padrao: %f\n",desvp); 
  fprintf(f,"#valor del standarderror: %f\n",standarderror);

  fprintf(f,"#valor de la media magnetica: %f\n",fabs(mag_media));
  fprintf(f,"#valor de la susceptibilidad: %f\n",Suscepti);
  
 // printf("valor del ruido: %f\n",s2); 
 // printf("valor do devio padrao: %f\n",desvp); 
